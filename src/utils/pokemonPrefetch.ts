...
export async function prefetchPokemonFromMessage(
  ...
  const typePrefetchPromises = pokemonTypes.slice(0, 2).map(type => // Limit to 2 types
    queryClient.prefetchQuery({
      ...
      queryFn: () => pokemonService.fetchType(type),
  ...
  if (message.toLowerCase().includes('random')) {
    ...
    prefetchPromises.push(
      queryClient.prefetchQuery({
        queryKey: ['pokemon', 'random', randomId],
        queryFn: () => pokemonService.fetchPokemon(randomId.toString()),
 ...
 */
export function isPokemonCached(queryClient: QueryClient, nameOrId: string | number): boolean {
  const data = queryClient.getQueryData(['pokemon', 'lookup', nameOrId]);
 ...
 */
export function getCachedPokemon(queryClient: QueryClient, nameOrId: string | number) {
